...
> The modular template is intended for plugins that are designed for Split Mode and modular plugin packaging from the start.
>
...
Use the modular template when a plugin is expected to provide optimal low-latency UX and correct project interaction in Split Mode as well as in a monolithic IDE. To start developing a split plugin, use the [IntelliJ Platform Modular Plugin Template](https://github.com/JetBrains/intellij-platform-modular-plugin-template).
