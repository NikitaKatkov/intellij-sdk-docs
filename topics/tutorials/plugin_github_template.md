...
> The modular template is intended for plugins that are designed for [Split Mode](remote_development.md) and modular plugin packaging from the start.
>
...
Use the modular template when a plugin is expected to provide optimal low-latency UX and correct project interaction in [Split Mode](remote_development.md) as well as in a monolithic IDE. To start developing a [split plugin](remote_development.md), use the [IntelliJ Platform Modular Plugin Template](https://github.com/JetBrains/intellij-platform-modular-plugin-template).
