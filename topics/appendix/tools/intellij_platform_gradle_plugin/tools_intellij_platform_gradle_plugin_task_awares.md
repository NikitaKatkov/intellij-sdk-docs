...
When you develop a plugin, you may want to check how it works in [split mode](remote_development.md), when one machine is running the backend part and another is running a frontend part (JetBrains Client) which connects to the backend.
...
[Split Mode](remote_development.md) requires the IntelliJ Platform in the version `241.14473` or later.
