...
1. Identify or create necessary plugin modules.
   ...
   *Expected outcome: a plugin consists of at least three modules, namely frontend, backend, and shared, plus possibly the existing code that resides in a root plugin module or in other submodules.*
...
2. Put the existing or newly written code into appropriate module types:
   ...
   *Expected outcome: your plugin is not functioning properly since the UI is now completely detached from the business logic. The code, though, is properly distributed between modules that do not communicate with each other yet. The root plugin module carries only the plugin.xml descriptor in its resources folder. All the code from it is extracted to one of the freshly created modules.*
...
5. After all infrastructure has been implemented, it is time to verify the feature behavior and polish it. Refer to [Introduction into Split Mode / Remote Development](remote_development.md) on how to manually test Split Mode, and check the monolithic IDE as well \- the behavior is expected to be exactly the same.
   *Expected outcome: the code is valid from the point of view of this guide, and the behavior is as expected in both Split Mode and a monolithic IDE*
6. Now that the general functionality works as expected, consider reviewing the list of frequently occurring problems and suggested solutions for them. Depending on the specifics of the feature, you might not necessarily need to tune the code.
...
7. Fix the split feature behavior and quality with unit and integration tests if you have not used the TDD approach earlier. See the [split-mode testing section](remote_development.md#how-to-run-tests-in-split-mode-with-gradle).
