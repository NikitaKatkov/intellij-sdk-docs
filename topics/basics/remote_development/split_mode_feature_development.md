...
# IMPLEMENTING A FEATURE FOR THE SPLIT MODE
...
4. Add the transport layer to connect your UI with the backend model
    ...
    * Call the RPC where the backend data is required
        ...
        3. ~~Instead, prefer introducing an intermediate abstraction layer that will hold an immediately ready to use state that is eventually consistent with the backend~~
            1. ~~For example, look at the Recently Edited Files feature that displays a list of files in the currently opened project that have recently been changed~~
            2. ~~Let’s consider two main sources of such changes: first is user tying in the file editor, and second is a global refactoring like rename is applied to a set of files~~
            3. ~~Typing happens on the frontend side, so we set up a typing listener there, and push the edited file into the **frontend recently edited files model**, which in turn sends this data to backend and achieves consistent state~~
            4. ~~Refactorings typicaclly happen on the backend side, so we set up a virtual file system listener there, and push the refactored files into the **backend recently edited files model,** which in turn sends this data to frontend and achieves consistent state~~
            5. ~~At some point data in frontend and backend models might be different, for instance after we processed the backend refactoring event, but due to latency have not yet sent it to frontend. In this case the user will see a slightly outdated state should they invoke the corresponding action at this very moment. Specifically recent files feature allows such behaviour. If your case is different, consider communicating the stale frontend state to the user and refresh the UI after it gets synchronized~~
            6. ~~If both frontend and backend may alter the shared state your feature relies on, consider having such model on both ends and sync their state depending on a particular feature needs~~
            7. ~~An example of such implementation you can find in the sources of IntelliJ project, namely in the Recent Files feature implementation, see [https://github.com/JetBrains/intellij-community/tree/1c3952828ff3af2d18f99a6721c48bb22f97bd57/platform/recentFiles](https://github.com/JetBrains/intellij-community/tree/1c3952828ff3af2d18f99a6721c48bb22f97bd57/platform/recentFiles)~~
        4. Consider using the existing platform abstraction for shared state as a reference [https://github.com/JetBrains/intellij-community/blob/1c3952828ff3af2d18f99a6721c48bb22f97bd57/platform/lang-impl/src/com/intellij/build/FlowWithHistory.kt](https://github.com/JetBrains/intellij-community/blob/1c3952828ff3af2d18f99a6721c48bb22f97bd57/platform/lang-impl/src/com/intellij/build/FlowWithHistory.kt)
...
6. Now the general functionality works as expected, consider reviewing the list of frequently occurring problems and suggested solutions for them. Depending on the feature specifics, you might not necessarily need to tune the code.
    * Handle reconnection: wrap RPC calls into *durable{...};* this wrapper will restart the call should a network error occur. Be careful with any side effects your code inside the durable block produces \- ideally, avoid them. Example [https://github.com/JetBrains/intellij-community/blob/1c3952828ff3af2d18f99a6721c48bb22f97bd57/platform/recentFiles/frontend/src/com/intellij/platform/recentFiles/frontend/model/RecentFileModelSynchronizer.kt](https://github.com/JetBrains/intellij-community/blob/1c3952828ff3af2d18f99a6721c48bb22f97bd57/platform/recentFiles/frontend/src/com/intellij/platform/recentFiles/frontend/model/RecentFileModelSynchronizer.kt)
    * ~~Synchronize Frontend and Backend code style settings, if any. Example: [https://github.com/JetBrains/intellij-community/blob/1c3952828ff3af2d18f99a6721c48bb22f97bd57/platform/code-style-api/src/com/intellij/psi/codeStyle/CodeStyleSettingsCustomDataSynchronizer.kt](https://github.com/JetBrains/intellij-community/blob/1c3952828ff3af2d18f99a6721c48bb22f97bd57/platform/code-style-api/src/com/intellij/psi/codeStyle/CodeStyleSettingsCustomDataSynchronizer.kt)~~
    * Register actions in proper plugin modules
        ...
        3. *(\*\*\*advanced\*\*\*) In some complicated cases action.update() method might require both frontend and backend entities available simultaneously. Such cases must be addressed individually depending on a specific feature description. There are examples of shared eventually synchronized state implementations in the IntelliJ Platform codebase, for instance in the [https://github.com/JetBrains/intellij-community/tree/1c3952828ff3af2d18f99a6721c48bb22f97bd57/platform/recentFiles](https://github.com/JetBrains/intellij-community/tree/1c3952828ff3af2d18f99a6721c48bb22f97bd57/platform/recentFiles) . There an eventually-consistent mutable shared state is implemented to make it possible to access the data model on both frontend and backend with no RPC calls required for the access*
