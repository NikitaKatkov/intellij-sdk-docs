...
See the [Split Mode feature guide](split_mode_feature_development.md) for the migration flow, the [RPC guide](remote_procedure_calls.md) for communication between modules, and the [API placement guide](frontend_backend_shared_apis.md) for common API placement guidance.
...
For how the IDE determines where a plugin can be installed and how backend/frontend plugin sets are synchronized, see [Plugin Management](plugin_management_in_remote_development.md).
